...
public class ErrorResponseEntity {
     ...
     */
    public static ResponseEntity buildToResponseEntity(int errorCode, String msg, int... statusCode) {
        ErrorResponseEntity responseEntity = new ErrorResponseEntity(errorCode, msg);
        if (statusCode != null && statusCode.length == 1) {
            return ResponseEntity.status(statusCode[0]).body(responseEntity);
        }
